...
class EditBooking extends React.Component{
    ...
    componentDidMount() {
        ...
        let prevBooking = this.props.location.state.booking;
            this.setState({
                trainerid: prevBooking.trainerid,
                location: prevBooking.location,
                coursename: prevBooking.coursename,
                startDatetime: new Date(prevBooking.startDatetime.seconds*1000),
                endDatetime: new Date(prevBooking.endDatetime.seconds*1000),
                id: prevBooking.id
            });
            // console.log((new Date(prevBooking.endDatetime.seconds*1000)), prevBooking.endDatetime, Date.now())
        }
    }
    ...
    handleStartDateChange = datetime => {
        ...
        // console.log(this.state.endDatetime, 'BRUHHH')
    ...
    validateForm(details, trainer){
        ...
        taughtCourses = taughtCourses.substring(0, taughtCourses.length-2);
                                    ...
                                    Promise.all(
                                        data.bookings.map(
                                            bookingid => db.collection('bookings').doc(bookingid).get().then(bookingRef => {
                                                //check
                                                if (bookingRef.id===this.state.id){
                                                    // console.log('Same booking, skipping check...');
                                                    return true;
                                                }
                                        ...
                                        if (isAvailable) {
                                            this.setState({formerror: ''});
                                            //EDIT TOO BOOKINGS DOCUMENT
                                            db.collection('bookings').doc(this.state.id).update({
                                                ...bookingData
                                            }).then(()=>{
    ...
    render() {
        // console.log(this.props.location.state.booking, 'PORORROROROOPS')
        // console.log(this.state.id, 'HIFJAFOJS')
        ...
        return (
            <Container>
                <Typography variant='h6' gutterBottom>Edit Booking</Typography>
